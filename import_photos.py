...
def main(username=None, password=None, lastmail=False):
    ...
    for msgId in data[0].split():
        ...
        try:
            ...
            logging.error('Error with mtime data: ' + str(e))
            continue
        ...
        for part in mail.walk():
            ...
            # using mimetypes to check if we are getting an image
            # that way we can get images of any type that mimetype can handle
            if bool(filename) and 'image' in mimetypes.guess_type (filename)[0]:
                filename = 'UH-{0}_{1}-{2} {3}'.format(mtime.date(), mtime.hour, mtime.minute, filename)
